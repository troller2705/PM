...
import fog_fragmentGlsl from "three/src/renderers/shaders/ShaderChunk/fog_fragment.glsl.js";
...
function TaskCard({ task, index, getUser, onEdit, onDelete }) {
  ...
  // Safely grab the MongoDB ID or fallback to the mock ID
              ...
              <span className={cn("px-2 py-0.5 text-xs font-medium rounded", TYPE_COLORS[task.task_type] || 'bg-slate-100 text-slate-600')}>
                ...
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end">
                    ...
                    <DropdownMenuItem className="text-red-600" onClick={() => onDelete(taskId)}>
              ...
              <div className="flex items-center justify-between mt-3">
                <div className="flex items-center gap-2">
                  ...
                  <Clock className="h-3 w-3" />
                        {formatLocalDate(task.due_date, 'MMM d')}
                </span>
                  )}
                </div>
...
export default function KanbanBoard({ tasks, users, onStatusChange, onEdit, onDelete }) {
  const getUser = (id) => users.find(u => (u._id || u.id) === id);
  ...
  const handleDragEnd = (result) => {
    ...
    if (destination.droppableId !== source.droppableId) {
      // Find the task using the safe MongoDB ID logic
      const task = tasks.find(t => String(t._id || t.id) === draggableId);
                          ...
                          >
                            <div className="space-y-3">
                              {columnTasks.map((task, index) => {
                                // Ensure we have a valid key for React's mapping requirement
                                const taskId = String(task._id || task.id);
                                return (
                                    <TaskCard
                                        key={taskId}
