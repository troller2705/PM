...
// 1. FIXED DATE PARSER: Forces the raw database string to local noon
// before doing any Gantt Chart math so it never rolls backwards!
const safeParseDate = (dateString, fallback = null) => {
  if (!dateString) return fallback;
  const localNoonStr = dateString.split('T')[0] + 'T12:00:00';
  const parsed = new Date(localNoonStr);
...
export default function GanttChart({ tasks = [], dependencies = [], users = [] }) {
  ...
  const { flatRows, minDate, maxDate, totalDays } = useMemo(() => {
    ...
    // 2. FIXED HIERARCHY MAP: Use MongoDB _id
    ...
    const flatten = (nodes, depth = 0) => {
      ...
      nodes.sort((a, b) => (a.order || 0) - (b.order || 0)).forEach(node => {
        const nodeId = node._id || node.id; // Safely extract ID
        ...
        // 3. FIXED LOOKUPS: Check MongoDB _id
        const assignee = users.find(u => (u._id || u.id) === node.assignee_id);
        const hasDeps = dependencies.some(d => d.task_id === nodeId || d.depends_on_task_id === nodeId);
        const isParent = node.children && node.children.length > 0;

        result.push({ task: node, depth, isParent, startPct, widthPct, assignee, hasDeps });

        if (isParent && !collapsedRows.has(nodeId)) {
            ...
            <TooltipProvider delayDuration={200}>
              <div className="relative pb-10">
                ...
                {flatRows.map(({ task, depth, isParent, startPct, widthPct, assignee, hasDeps }) => {
                  const taskId = task._id || task.id; // Safely extract ID for rendering
                      ...
                      <div key={taskId}
                        ...
                        <div className="w-[300px] shrink-0 px-4 flex items-center gap-1.5 bg-inherit border-r border-slate-100 shadow-[1px_0_0_rgba(0,0,0,0.02)]">
                          ...
                          {isParent ? (
                              ...
                              >
                                {collapsedRows.has(taskId) ? <ChevronRight size={14} /> : <ChevronDown size={14} />}
                              </button>
                          ) : (
